...
COPY pg_hba.conf /usr/share/postgresql/10/pg_hba.conf.sample
COPY postgresql.conf /usr/share/postgresql/10/postgresql.conf.sample
...
RUN apt-get update \
    ...
    && unzip pgtap.zip \
    && apt-get install -y postgresql-server-dev-10 postgresql-common \
